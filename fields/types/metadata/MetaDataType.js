var FieldType = require('horizontec-keystone/fields/types/Type');
var numeral = require('numeral');
var util = require('util');
var utils = require('keystone-utils');
var keystone = require('horizontec-keystone');
var addPresenceToQuery = require('horizontec-keystone/fields/utils/addPresenceToQuery');
 ...
 * @api public
 */
function metadata (list, path, options) {
	this._nativeType = [{text: String, image: String}];
	...
	metadata.super_.call(this, list, path, options);
}


metadata.properName = 'MetaData';
util.inherits(metadata, FieldType);

/**
 * Formats the field value
 */
// metadata.prototype.format = function (item, format, separator) {
 ...
 */
metadata.prototype.validateInput = function (data, callback) {
	var value = this.getValueFromData(data);
	console.log('Validate input value: ', value) // ['{text: '', image: ''}',''] or '{text: '', image: ''}'
	var result = true;
 ...
 */
metadata.prototype.validateRequiredInput = function (item, data, callback) {
	var value = this.getValueFromData(data);
	var result = false;
 ...
 * Deprecated
 */
metadata.prototype.inputIsValid = function (data, required, item) {
	var value = this.getValueFromData(data);
	console.log('inputIsValid? value: ', data)
	if (required) {
 ...
 */
metadata.prototype.updateItem = function (item, data, callback) {
	var value = this.getValueFromData(data);
	console.log('updateItem value1: ', value)
	...
	value = value.map(function (num) {
		return ({text: num, image:num})
	})

	item.set(this.path, value);
	console.log('updateItem value4: ', value)
