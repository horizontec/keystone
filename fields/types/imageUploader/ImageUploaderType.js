...
var debug = require('debug')('keystone:fields:imageUploader');

/**
 * File FieldType Constructor
 */
function imageUploader (list, path, options) {
	this._underscoreMethods = ['format', 'upload', 'remove', 'reset'];
	this._fixedSize = 'full';

	if (!options.storage) {
		throw new Error('Invalid Configuration\n\n'
			+ 'ImageUploader fields (' + list.key + '.' + path + ') require storage to be provided.');
	}
	this.storage = options.storage;
	imageUploader.super_.call(this, list, path, options);
}
imageUploader.properName = 'ImageUploader';
util.inherits(imageUploader, FieldType);

/**
 * Registers the field on the List's Mongoose Schema.
 */
imageUploader.prototype.addToSchema = function (schema) {
 ...
 * Uploads a new file
 */
imageUploader.prototype.upload = function (item, file, callback) {
	var field = this;
 ...
 */
imageUploader.prototype.reset = function (item) {
	var value = {};
	Object.keys(this.storage.schema).forEach(function (path) {
		value[path] = null;
	});
...
imageUploader.prototype.remove = function (item) {
 ...
 */
imageUploader.prototype.format = function (item) {
 ...
 */
imageUploader.prototype.isModified = function (item) {
	var modified = false;
...
function validateInput (value) {
	...
	if (typeof value === 'string' && /^(select\:)|(upload\:)|(delete$)/.test(value)) return true;
 ...
 */
imageUploader.prototype.validateInput = function (data, callback) {
 ...
 */
imageUploader.prototype.validateRequiredInput = function (item, data, callback) {
 ...
 */
imageUploader.prototype.updateItem = function (item, data, files, callback) {
	// Process arguments
	...
	if (typeof value === 'string' && value.substr(0, 7) === 'select:') {
		uploadedFile = {
			mimetype: 'image',
			filename: value.substr(7),
		};
		item.set(this.path, uploadedFile);
